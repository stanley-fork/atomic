...
/// 6. Compute semantic edges (unless deferred)
...
pub async fn process_tagging_only(
    ...
    let supported_params: Option<Vec<String>> = if provider_config.provider_type == ProviderType::OpenRouter {
        ...
        let capabilities = get_model_capabilities(move || {
            atomic_core::Database::open(&db_path)
...
pub fn spawn_embedding_task_single(
    ...
    std::thread::spawn(move || {
        ...
        let app_handle_embed = app_handle.clone();
        let app_handle_tag = app_handle.clone();
        ...
        rt.block_on(async move {
            let embed_handle = tokio::spawn(async move {
                ...
                let _ = app_handle_embed.emit("embedding-complete", payload);
            });
            ...
            let tag_handle = tokio::spawn(async move {
                ...
                let _ = app_handle_tag.emit("tagging-complete", payload);
            });
...
/// Embedding and tagging run in parallel per atom — semaphores control concurrency.
            ...
            if success {
                ...
                let _ = app_handle.emit(
                    ...
                    EmbeddingCompletePayload {
                        atom_id: atom_id.clone(),
                        status: "failed".to_string(),
                        error: Some("Failed to store embeddings in DB".to_string()),
                    },
                );
            }
        }
        ...
        for atom_id in &failed_atom_ids {
            ...
            let _ = app_handle.emit(
                ...
                EmbeddingCompletePayload {
                    atom_id: atom_id.clone(),
                    status: "failed".to_string(),
                    error: Some("Embedding API call failed after retries".to_string()),
                },
            );
        }
